...
État au 25 septembre 2026 : la version initiale `96faa9b` a été mise en production. La réception de `page_view` et de deux événements `book_outbound_click` a été vérifiée dans Google Analytics : `book_id=la_bascule` apparaît deux fois, et `retailer=fnac` et `retailer=amazon` une fois chacun. L’utilisateur a accepté les conditions Google Analytics dans le navigateur. La nouvelle présentation de `/livre` (`9d1a99e`) a été vérifiée en production ; l’ajout du livre aux menus (`64726bc`) est également en ligne. La modification décrite ci-dessous pour supprimer l’attente d’un choix de cookies sur les pages de redirection est préparée dans le code ; son déploiement reste à confirmer.

Le code du compteur agrégé est publié mais reste désactivé : `BOOK_COUNTER_ENABLED` et `NEXT_PUBLIC_BOOK_COUNTER_ENABLED` ne sont pas activés. La migration Supabase et ses restrictions d’accès sont vérifiées. Aucun jeton n’a encore été généré ; l’approbation demandée dans le navigateur reste en attente. Les déploiements du site ne valent pas activation du comptage.
...
- Aucun script de mesure Google externe n’est chargé avant acceptation. Le choix est conservé dans `localStorage` sous `cookie-consent-v1`, et en mémoire si le stockage est indisponible. La bannière et le bouton « Cookies » restent disponibles sur `/livre` et le reste du site, hors pages transitoires `/livre/fnac` et `/livre/amazon`.
...
- Les routes `/livre/fnac` et `/livre/amazon` répondent HTTP 200 avec une page qui redirige automatiquement dès que JavaScript s’exécute. Elles n’attendent aucun choix de cookies et n’affichent pas la bannière, quel que soit l’état du compteur. Sans acceptation préalable, y compris après refus, le départ est immédiat et aucun événement GA4 n’est émis. Le lien de secours reste utilisable sans JavaScript.
- Avec acceptation préalable, un identifiant GA4 valide et une balise disponible, le site demande l’envoi de `book_outbound_click` avant le départ. La redirection attend le rappel de la balise, avec une limite indépendante de 1,5 seconde si la balise est bloquée ou ne répond pas. Ce délai ne garantit pas la réception de l’événement par Google. Une balise absente, une erreur ou une configuration manquante ne bloque pas le départ.
- Le compteur reste optionnel : désactivé, la destination du navigateur est directement l’URL fixe du libraire ; activé, elle devient `/go/fnac` ou `/go/amazon`, qui répond HTTP 303. Aucune redirection HTTP 301 n’est utilisée par ces routes ; une redirection serveur à l’entrée de `/livre/fnac` ou `/livre/amazon` empêcherait l’exécution préalable du code GA4.
...
- Le rappel de la balise et le délai ne peuvent provoquer qu’une seule navigation. Les rediffusions d’effet React et les notifications répétées de consentement ne doivent pas doubler l’événement. Le lien de secours annule la redirection automatique encore en attente.
...
| `NEXT_PUBLIC_BOOK_COUNTER_ENABLED=true` | Construction du client : fait passer les redirections et liens de secours par `/go` et affiche les informations sur le compteur dans la politique de confidentialité. Le départ automatique sans dialogue de cookies ne dépend pas de cet indicateur. |
...
- [x] Nouvelle présentation de `/livre` (`9d1a99e`) vérifiée : HTTP 200, contenu complet, ressources graphiques, liens internes conservant les UTM et métadonnées indexables. Code du compteur présent mais désactivé.
- [x] Lien du livre ajouté aux menus ordinateur et mobile ; version `64726bc` en ligne sur Render.
- [ ] Déployer et vérifier le départ automatique des routes HTTP 200 `/livre/fnac` et `/livre/amazon` sans dialogue de cookies : immédiat sans consentement, après rappel GA4 ou 1,5 seconde au maximum avec acceptation préalable. Confirmer l’absence d’événement GA4 pour un choix inconnu ou refusé.
...
Avant la modification du départ automatique sans choix de cookies, la construction complète avec l’identifiant réel `G-5EBLGEKCHV` et `NEXT_PUBLIC_BOOK_COUNTER_ENABLED=true` avait réussi après correction de la concurrence entre le lien de secours et la redirection automatique ; les tests de liens, les 10 tests du compteur et la suite de consentement passaient. Les 25 tests de liens passent avec le nouveau comportement : ils couvrent notamment le départ immédiat sans GA4 pour chaque libraire, avec consentement inconnu ou refusé et indicateur de compteur absent ou faux. La nouvelle construction complète et la suite de consentement ont réussi. Les deux routes ont été contrôlées en HTTP 200 sans en-tête Location ; les redirections automatiques vers les fiches Amazon et Fnac ont été vérifiées dans Chrome depuis une nouvelle origine locale sans choix de cookies enregistré. La validation de production reste à confirmer.
